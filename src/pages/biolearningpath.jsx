...
function BioLearningPath() {
  ...
  useEffect(() => {
    const fetchAllLessons = async () => {
      try {
        const res = await axios.get("/api/biocards",
        {
  ...
  const renderCard = () => {
    if (cardNum > -1 && cardNum < lesson.length) {
      ...
      return <Intro lessonIndex={lessonid} course="bio"></Intro>;
    } else if (cardNum === lesson.length) {
      return <Conclusion lessonIndex={lessonid} course="bio"></Conclusion>;
    }
  };

  return (
    <ThemeProvider theme={theme} key="biolearn-1">
      <NavBar key="biolearn-2" />
      <BigGrid key="biolearn-3">
        <ChevronButton
          key="biolearn-4"
        ...
        >
          <CardWrapper fadeOut={fadeOut} key="biolearn-7">
            {renderCard()}
          </CardWrapper>
        </CardHolder>
        <ChevronButton
